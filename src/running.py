...
from datasets.dataset import (
    ClassiregressionDatasetWithEmb, collate_superv_with_emb
)
...
def pipeline_factory(config):
    """
    Returns the Dataset class, collate function, and Runner class for TimeCMA.
    
    TimeCMA always uses embeddings for cross-modal alignment.
    """
    task = config['task']

    if task not in ("classification", "regression"):
        raise NotImplementedError(f"Task '{task}' not implemented. TimeCMA supports 'classification' and 'regression'.")
    ...
    dataset_class = partial(
        ClassiregressionDatasetWithEmb,
        embedding_dir=config.get('embedding_dir', './Embeddings'),
        dataset_name=dataset_name,
    )
    return dataset_class, collate_superv_with_emb, SupervisedRunnerWithEmb
...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    if condition:
        ...
        np.savez(
            pred_filepath,
            **{k: np.array(v, dtype=object) for k, v in per_batch.items()}
        )
...
class SupervisedRunnerWithEmb(BaseRunner):
    """
    Supervised runner for TimeCMA model with embedding support.
    
    This runner passes embeddings to the model during forward pass,
    enabling cross-modal alignment between time series and text features.
    """

    def __init__(self, *args, **kwargs):
        super(SupervisedRunnerWithEmb, self).__init__(*args, **kwargs)
    ...
    def train_epoch(self, epoch_num=None):
        ...
        for batch in self.dataloader:
            # Unpack batch with embeddings
            X, targets, embeddings, padding_masks, IDs = batch
            ...
            predictions = self.model(X.to(self.device), padding_masks, embeddings)
