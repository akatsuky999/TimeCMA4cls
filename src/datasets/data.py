...
try:
    from sktime.utils import load_data  # older sktime versions (<0.16)
    load_from_tsfile_to_dataframe = load_data.load_from_tsfile_to_dataframe
except ImportError:  # pragma: no cover - depends on installed sktime version
    # In newer releases the loader lives under `sktime.datasets`
    from sktime.datasets import load_from_tsfile_to_dataframe

    class _LoadDataShim:
        load_from_tsfile_to_dataframe = staticmethod(load_from_tsfile_to_dataframe)

    load_data = _LoadDataShim()
...
class TSRegressionArchive(BaseData):
    ...
    def load_single(self, filepath):
        ...
        lengths = df.map(lambda x: len(x)).values  # (num_samples, num_dimensions) array containing the length of each series
        ...
        if np.sum(horiz_diffs) > 0:  # if any row (sample) has varying length across dimensions
            ...
            df = df.map(subsample)  # TODO: this addresses a very specific case (PPGDalia)

        if self.config['subsample_factor']:
            df = df.map(lambda x: subsample(x, limit=0, factor=self.config['subsample_factor']))
