"""
Cross-Modal Alignment module from TimeCMA project.
...
import numpy as np
from .TS_Pos_Enc import Transpose, get_activation_fn


class CrossModal(nn.Module):
    """
    Cross-modal attention module for aligning time series features with text embeddings.
    """
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, 
        ...
        if self.res_attention:
            for mod in self.layers:
                output, scores = mod(q, k, v, prev=scores, 
                                    key_padding_mask=key_padding_mask, 
                                    attn_mask=attn_mask)
            return output
        else:
            for mod in self.layers:
                output = mod(q, k, v, key_padding_mask=key_padding_mask, 
                           attn_mask=attn_mask)
            return output
...
class _ScaledDotProductAttention(nn.Module):
    """
    Scaled Dot-Product Attention module with optional residual attention.
    """
